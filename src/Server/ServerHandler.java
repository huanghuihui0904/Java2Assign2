...
public class ServerHandler implements Runnable {
  ...
  public ServerHandler(Socket socket, Store myServer) throws IOException {
    ...
    System.out.println("Connected Player 1 in init");
...
new BeforeEnd(myServer);
    try {
      ...
      while (true) {
        ...
        if (myServer.state.equals("p")) {

          responseOutput = new PrintStream(socket.getOutputStream(), true);

          responseOutput.println("p1in" + myServer.round);
          break;
        } else {
          responseOutput = new PrintStream(socket.getOutputStream(), true);

          responseOutput.println("p1lock");
        }
      }
      while (true) {
        ...
        if (myServer.step % 2 == 0) {
          ...
          if (strPrev != myServer.strGameState) {
            System.out.println("p1 update detected!");
